...
// executeConventionOp executes a convention operation using the Executor pattern:
//  1. Write the mutation to local JSONL (durability write-ahead).
//  2. Call exec.Execute() to send to campfire with validated tags and antecedents.
//
// The JSONL record uses a locally-generated message ID (from message.NewMessage).
// The campfire message ID generated by the executor is different — this is accepted
// by design (D6). The primary operation tag (work:<operation>) is recorded in JSONL
// for operation tracking.
//
// Returns the locally-generated message (for its ID) and the campfire ID.
// On executor failure the mutation is buffered to pending.jsonl so it can be
// retried on the next successful send (same resilience policy as sendToProjectCampfire).
func executeConventionOp(agentID *identity.Identity, s store.Store, exec *convention.Executor, decl *convention.Declaration, argsMap map[string]any) (*message.Message, string, error) {
	...
	// Phase 1 — build a local message for JSONL write-ahead.
	// The payload is the JSON encoding of argsMap (same data the executor will send).
	...
	// Phase 2 — execute via convention executor (optional — skipped if no campfire).
	if !hasCampfire {
		return msg, "", nil
	}
	...
	// Executor succeeded — update sync cursor and flush pending.
	if projectDir, ok := readyProjectDir(); ok {
		if markErr := rdSync.MarkSynced(projectDir, msg.ID, time.Now().UnixNano()); markErr != nil {
			fmt.Fprintf(os.Stderr, "warning: could not update sync cursor: %v\n", markErr)
		}
...
// executeConventionOpToCampfire is like executeConventionOp but sends to an explicit
// campfireID rather than looking up the project campfire. Used by depRemoveCmd which
// must send the unblock message to the campfire containing the original block message.
func executeConventionOpToCampfire(agentID *identity.Identity, s store.Store, exec *convention.Executor, decl *convention.Declaration, campfireID string, argsMap map[string]any) (*message.Message, string, error) {
	...
	// Phase 2 — execute via convention executor.
	ctx := context.Background()
	if _, execErr := exec.Execute(ctx, decl, campfireID, argsMap); execErr != nil {
		fmt.Fprintf(os.Stderr, "warning: campfire send failed (buffered to pending.jsonl): %v\n", execErr)
		if bufErr := bufferToPending(msg, campfireID, string(payloadJSON), []string{primaryTag}, nil); bufErr != nil {
